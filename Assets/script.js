...
function weatherInformation(api) {
  ...
  .then(function (data) {
    ...
    console.log(data)
    currentUV.textContent = data.uvi
...
inputForm.addEventListener('submit', function(event){
  ...
  var futureCities = []
  if (futureCities[0]) {
    futureCities.concat(cityInputEl.value)
  } else {
    futureCities.push(cityInputEl.value)
  }
  ...
  if (localCities) {
    if (!localCities.includes(cityInputEl.value)) {
      localStorage.setItem('cities', futureCities.push(cityInputEl.value))
    }
  } else {
...
function pastCities(name) {
  var pastCities = document.createElement('li');
  pastCities.classList.add('past-inputs', 'bg-light', 'p-0');

  var pastCitiesButton = document.createElement('button');
  pastCitiesButton.classList.add('border-0', 'bg-light', 'w-100', 'p-2', 'round-border');
  // pastCitiesButton.textContent = 
  pastCities.append(pastCitiesButton);

  cities.append(pastCities)
}